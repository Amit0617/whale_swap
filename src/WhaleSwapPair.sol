...
contract WhaleSwapPair is WhaleSwapERC20 {
    ...
    function mint(address to) external lock returns (uint256 liquidity) {
        ...
        uint256 _totalSupply = totalSupply; // gas savings, must be defined here since totalSupply can be updated
    ...
    function burn(address to) external lock returns (uint256 amount0, uint256 amount1) {
        ...
        bool feeOn = _mintFee(_reserve0, _reserve1);
        uint256 _totalSupply = totalSupply; // gas savings, must be defined here since totalSupply can be updated
        amount0 = (balance0 * liquidity) / _totalSupply; // using balances ensures pro-rata distribution
        amount1 = (balance1 * liquidity) / _totalSupply; // using balances ensures pro-rata distribution
    ...
    function swap(uint256 amount0Out, uint256 amount1Out, address to, bytes calldata data) external lock {
        ...
        require(amount0In > 0 || amount1In > 0, "WhaleSwap: INSUFFICIENT_INPUT_AMOUNT");
        {
            ...
            require(
                (balance0Adjusted * balance1Adjusted) >= (uint256(_reserve0) * _reserve1) * 1000 ** 2, "WhaleSwap: K"
            );
        }
